...
eve(entra(var_Agent,var_Type)):-assert(type(var_Agent,var_Type)),write(' Entra '),write(var_Agent),write(' ('),write(var_Type),write(') '),nl,(var_Type=seller->a(add_seller(var_Agent))).

eve(esce(var_Agent,var_Type)):-format('Il ~w ~w ha deciso di andare via.~n',[var_Type,var_Agent]).

eve(trading(var_Buyer,var_Saldo,var_Eta)):-var_Eta<18,write(var_Buyer),write(' sei minorenne!!!'),a(message(var_Buyer,send_message(esci,var_Me))),retractall(type(var_Buyer,buyer)),nl.

eve(trading(var_Buyer,var_Saldo,var_Eta)):-var_Eta>=18,write(var_Buyer),write(' vorrebbe acquistare qualcosa!!!'),nl,a(avvisa_seller(var_Buyer,var_Saldo)).

a(avvisa_seller(var_Buyer,var_Saldo)):-findall(var_Agent,(seller(var_Agent),\+matching(var_Buyer,var_Agent)),var_Sellers),a(send_message_to_sellers(var_Sellers,var_Buyer,var_Saldo)).

a(send_message_to_sellers([],var__,var__)):-write('Non ci sono altri venditori'),type(var_Buyer,buyer),a(message(var_Buyer,send_message(esci,var_Me))).

a(send_message_to_sellers([var_Agent|var_Rest],var_Buyer,var_Saldo)):-format('From Buyer Agent: ~w~n',var_Buyer),format('Sending message to Seller Agent: ~w~n',var_Agent),a(message(var_Agent,send_message(buyer_disponibile(var_Buyer,var_Saldo),var_Me))).

nonempty(var_Lista):-var_Lista=[var__|var__].

a(add_seller(var_Agent)):-assert(seller(var_Agent)),write(var_Agent),write(', un venditore!'),nl.

eve(acquisto_effettuato([var_Nome,var_Costo],var_Buyer)):-check_match([var_Nome,var_Costo],var_Buyer).

check_match([var_Nome,var_Costo],var_Buyer):-matching(var_Buyer,var_Seller),write('Acquisto effettuato correttamente: '),write(var_Nome),write(' da '),write(var_Buyer),nl,a(message(var_Seller,send_message(acquisto_done([var_Nome,var_Costo]),var_Me))).

check_match([var_Nome,var_Costo],var_Buyer):- \+matching(var_Buyer,var_Seller),write('I due agenti non stavano trattando!'),nl.

eve(match(var_Buyer,var_Seller)):-format('Il venditore ~w sta trattando con il cliente ~w . ~n ',[var_Seller,var_Buyer]),asserta(matching(var_Buyer,var_Seller)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_359785,var_Ontology,_359789),_359779),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_359823)),a(message(var_Ag,propose(var_A,[_359823],var_AgI))),retractall(ext_agent(var_Ag,_359861,var_Ontology,_359865)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_359659,var_Ontology,_359663),_359653),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_359729,var_Ontology,_359733)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_359547,var_Ontology,_359551),_359541),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_359603,var_Ontology,_359607)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_359111),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_359145),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_358927).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_358775),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_358723),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_358599,_358601)),clause(agente(_358621,_358623,_358625,var_S),_358617),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_358393,_358395,_358397,var_S),_358389),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_358493,[]),repeat,read(_358493,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_358493).
